#!/usr/bin/env python3
"""
Générateur Avancé de Sprites HoMM3 - Version Corrigée
...
class HoMM3AdvancedSpriteGenerator:
        ...
        for variation in range(3):
            ...
            for y in range(self.sprite_size):
                for x in range(self.sprite_size):
                    ...
                    final_color = (
                        int(base_color[0] * gradient + wave * 15),
                        int(base_color[1] * gradient + wave * 12),
                        int(base_color[2] * gradient + wave * 8)
                    )
    ...
    def _draw_unit_sprite(self, pixels, unit_name, direction, frame, unit_data):
        ...
        colors = unit_data
        center_x = self.sprite_size // 2
        center_y = self.sprite_size // 2
        ...
        if unit_name == 'swordsman':
            self._draw_swordsman(pixels, (center_x, center_y), colors, direction, frame)
        elif unit_name == 'archer':
            self._draw_archer(pixels, (center_x, center_y), colors, direction, frame)
        elif unit_name == 'knight':
            self._draw_knight(pixels, (center_x, center_y), colors, direction, frame)
        elif unit_name == 'goblin':
            self._draw_goblin(pixels, (center_x, center_y), colors, direction, frame)
        elif unit_name == 'skeleton':
            self._draw_skeleton(pixels, (center_x, center_y), colors, direction, frame)
    ...
    def _draw_archer(self, pixels, center, colors, direction, frame):
        ...
        # Tête
        head_y = center[1] - 20
        self._draw_circle(pixels, (center[0], head_y), 7, colors['skin'])
        
        # Arc
        if direction in ['east', 'northeast']:
            bow_start = (center[0] + 15, center[1] - 10)
            bow_end = (center[0] + 25, center[1] - 5)
        elif direction in ['west', 'northwest']:
            bow_start = (center[0] - 25, center[1] - 10)
            bow_end = (center[0] - 15, center[1] + 5)
        else:
            bow_y = center[1] - 15
            bow_start = (center[0], bow_y - 12)
            bow_end = (center[0], bow_y + 12)
    ...
    def _draw_knight(self, pixels, center, colors, direction, frame):
        ...
        # Tête
        head_y = center[1] - 30
        self._draw_circle(pixels, (center[0], head_y), 10, colors['skin'])
        
        # Casque
        helmet_y = head_y - 10
        self._draw_circle(pixels, (center[0], helmet_y), 12, colors['metal'])
        
        # Lance
        if direction in ['east', 'northeast']:
            lance_start = (center[0] + 25, center[1] - 15)
            lance_end = (center[0] + 40, center[1] - 25)
        elif direction in ['west', 'northwest']:
            lance_start = (center[0] - 40, center[1] - 25)
            lance_end = (center[0] - 25, center[1] + 25)
        else:
            lance_y = center[1] - 20
            lance_start = (center[0], lance_y - 18)
            lance_end = (center[0], lance_y + 18)
    ...
    def _draw_goblin(self, pixels, center, colors, direction, frame):
        ...
        # Tête
        head_y = center[1] - 15
        self._draw_circle(pixels, (center[0], head_y), 6, colors['skin'])
        
        # Oreilles
        ear_y = head_y - 8
        self._draw_circle(pixels, (center[0] - 4, ear_y), 3, colors['skin'])
        self._draw_circle(pixels, (center[0] + 4, ear_y), 3, colors['skin'])
    ...
    def _draw_skeleton(self, pixels, center, colors, direction, frame):
        ...
        # Corps
        body_y = center[1] + 5
        self._draw_circle(pixels, (center[0], body_y), 10, colors['secondary'])
        self._draw_circle(pixels, (center[0], body_y + 8), 8, colors['secondary'])
        
        # Bras
        arm_y = body_y - 5
        self._draw_line(pixels, (center[0] - 10, arm_y), (center[0] - 5, arm_y + 5), colors['bones'], 2)
        self._draw_line(pixels, (center[0] + 10, arm_y), (center[0] + 5, arm_y + 5), colors['bones'], 2)
    ...
    def generate_ui_elements(self):
        ...
        # Bordure dorée
        self._draw_border(pixels, 0, 0, 300, 200, self.colors['ui_border'])
    ...
    def _draw_hero_portrait(self, pixels, hero_name, hero_data):
        ...
        if hero_name == 'knight':
            # Armure complète
            self._draw_circle(pixels, (center_x, center_y), 25, colors['armor'])
            self._draw_circle(pixels, (center_x, center_y - 25), 15, colors['skin'])  # Tête
            self._draw_circle(pixels, (center_x, center_y - 35), 12, colors['hair'])  # Cheveux
            
            # Casque
            helmet_y = center_y - 35
            self._draw_circle(pixels, (center_x, helmet_y), 18, colors['armor'])
            self._draw_circle(pixels, (center_x, helmet_y), 10, colors['skin'])
            
        elif hero_name == 'wizard':
            # Robe
            self._draw_circle(pixels, (center_x, center_y), 22, colors['robe'])
            self._draw_circle(pixels, (center_x, center_y - 20), 15, colors['skin'])  # Tête
            self._draw_circle(pixels, (center_x, center_y - 28), 15, colors['hair'])  # Cheveux
            
            # Chapeau
            hat_y = center_y - 8
            self._draw_circle(pixels, (center_x, hat_y), 20, colors['robe'])
            
        elif hero_name == 'ranger':
            # Tenue de cuir
            self._draw_circle(pixels, (center_x, center_y), 20, colors['leather'])
            self._draw_circle(pixels, (center_x, center_y - 18), 12, colors['skin'])  # Tête
            self._draw_circle(pixels, (center_x, center_y - 28), 12, colors['hair'])  # Cheveux
            
            # Capuche
            hood_y = center_y - 8
            self._draw_circle(pixels, (center_x, hood_y), 15, colors['leather'])
